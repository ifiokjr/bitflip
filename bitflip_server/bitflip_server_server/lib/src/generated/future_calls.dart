...
import '../colour/colour_indexer_future_call.dart' as _iaq92ckg;
...
class FutureCalls extends _is.FutureCallDispatch<_FutureCallRef> {
  ...
  void initialize(
    ...
    var registeredFutureCalls = <String, _is.InvokableFutureCall>{
      'ColourIndexerScanFutureCall': ColourIndexerScanFutureCall(),
    };
...
class _FutureCallRef {
  ...
  late final colourIndexer = _ColourIndexerFutureCallDispatcher(
    _invokeFutureCall,
  );
}

class _ColourIndexerFutureCallDispatcher {
  _ColourIndexerFutureCallDispatcher(this._invokeFutureCall);

  final _InvokeFutureCall _invokeFutureCall;

  Future<void> scan() {
    return _invokeFutureCall(
      'ColourIndexerScanFutureCall',
      null,
    );
  }
}

class ColourIndexerScanFutureCall extends _is.FutureCall
  ...
  _ida.Future<void> invoke(
    ...
    await _iaq92ckg.ColourIndexerFutureCall().scan(session);
  }
}
