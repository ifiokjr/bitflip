...
The deterministic controller, game snapshot, section state, permissionless settlement, fixed custody, atomic paid-flip path, fixed owner fee share, policy-version binding, and authenticated colour event are implemented in economy ABI version 7. A mainnet reward-bearing release remains blocked on all of the following:
...
4. Real-SBF tests record compute for one and 16 pixels, first-time associated token-account creation, and existing token-account transfers. On 2026-09-19, the Pina 0.19 Surfpool suite recorded 443 invocations: flips peaked at 25,744 CU, the suite-wide p95 was 30,714 CU, and vault creation was the 53,214-CU maximum. Repeat this benchmark against the final staged artifact and network configuration before mainnet.
