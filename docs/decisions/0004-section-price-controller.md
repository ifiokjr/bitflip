...
The deterministic controller, game snapshot, section state, permissionless settlement, fixed custody, atomic paid-flip path, fixed owner fee share, and policy-version binding are implemented in economy ABI version 6. A mainnet reward-bearing release remains blocked on all of the following:
