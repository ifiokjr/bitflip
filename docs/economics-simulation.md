...
This remains a correctness stress test, not a validator-throughput benchmark. ABI version 5 removes the shared writable `GameState` counter, global-treasury payment, and owner-wallet payment from `FlipPixels`; the game and configuration are read-only, while pixels, controller state, BIT custody, protocol fees, and owner receivables are isolated in the section shard. Aggregate flip telemetry must therefore be indexed from confirmed section transactions rather than trusted from `GameState.total_flips`.
