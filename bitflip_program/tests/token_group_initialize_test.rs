...
>(
	...
	for ii in 0..4 {
		...
		let group = get_pda_mint(TokenMember::Bit).0;
		...
		let group_redaction = create_insta_redaction(group, "group:pubkey");
		let mint_account_data = rpc.get_account_data(&mint).await?;
		...
		insta::assert_compact_json_snapshot!(mint_account, {
		  ...
		  ".info.extensions[4].state.updateAuthority" =>	insta::dynamic_redaction(treasury_redaction.clone()),
		  ".info.extensions[4].state.group" =>	insta::dynamic_redaction(group_redaction.clone()),
