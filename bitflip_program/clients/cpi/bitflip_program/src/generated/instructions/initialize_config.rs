...
/// CPI call for the `initialize_config` instruction.
#[derive(Clone, Copy, Debug)]
#[must_use = "the CPI has no effect until invoke or invoke_signed is called"]
pub struct InitializeConfig<'account> {
	/// CPI account `payer`.
	...
	/// Instruction arguments encoded and sent as CPI data for `initialize_config`.
	pub ix: InitializeConfigIx,
}

/// Instruction arguments for the `initialize_config` CPI call.
#[derive(Clone, Copy, Debug)]
...
impl InitializeConfigIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 3], ProgramError> {
		let mut data = [0u8; 3];
		data[..2].copy_from_slice(&INITIALIZE_CONFIG_DISCRIMINATOR);
...
impl<'account> InitializeConfig<'account> {
...
const INITIALIZE_CONFIG_DISCRIMINATOR: [u8; 2] = [0, 0];
