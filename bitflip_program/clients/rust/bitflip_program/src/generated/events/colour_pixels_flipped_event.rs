//! This code was AUTOGENERATED by `pina_codama_renderer`.
//! Please DO NOT EDIT THIS FILE directly.
//! Regenerate it from Codama IDLs instead.
#![allow(
	clippy::doc_lazy_continuation,
	clippy::empty_line_after_doc_comments,
	clippy::new_without_default,
	clippy::too_many_arguments
)]

#[allow(clippy::len_without_is_empty)]
#[derive(pina::PinaPod)]
#[pinapod(crate = pina::pinapod, no_inherent)]
pub struct ColourPixelsFlippedEvent {
	pub discriminator: u8,
	pub migration_version: u8,
	pub player: solana_pubkey::Pubkey,
	pub policy_version: u64,
	pub revision: u64,
	pub coordinates: [u8; 32],
	pub game_index: u8,
	pub section_index: u8,
	pub count: u8,
	pub colour: u8,
}

pub const COLOUR_PIXELS_FLIPPED_EVENT_DISCRIMINATOR: u8 = 1u8;

pub const COLOUR_PIXELS_FLIPPED_EVENT_MIGRATION_VERSION: u8 = 0u8;

impl ColourPixelsFlippedEvent {
	/// Exact size of the current event representation.
	pub const LEN: usize = core::mem::size_of::<ColourPixelsFlippedEventZc>();

	/// Read one current-version event record from transaction-log bytes.
	///
	/// Logs carry the record base64-encoded after `Program data: `; pass the decoded bytes here. Historical and future records are rejected; `try_from_bytes` tells them apart, and `project_from_bytes` projects historical records when this client ships their transitions.
	pub fn from_bytes(
		data: &[u8],
	) -> Result<&ColourPixelsFlippedEventZc, solana_program_error::ProgramError> {
		let event = <Self as pina::PinaPodFixed>::read_exact(data)
			.map_err(|_| solana_program_error::ProgramError::InvalidArgument)?;
		if event.discriminator != COLOUR_PIXELS_FLIPPED_EVENT_DISCRIMINATOR {
			return Err(solana_program_error::ProgramError::InvalidArgument);
		}
		if event.migration_version != COLOUR_PIXELS_FLIPPED_EVENT_MIGRATION_VERSION {
			return Err(solana_program_error::ProgramError::InvalidArgument);
		}
		Ok(event)
	}
}

/// Why `ColourPixelsFlippedEvent::try_from_bytes` rejected event bytes.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ColourPixelsFlippedEventVersionError {
	/// The bytes do not decode as this event's layout at all.
	InvalidData,
	/// The envelope names this event but the stored version is newer than this client's schema: upgrade this client.
	Future { stored: u8 },
}

impl core::fmt::Display for ColourPixelsFlippedEventVersionError {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		match self {
			Self::InvalidData => write!(f, "invalid ColourPixelsFlippedEvent event data"),
			Self::Future { stored } => write!(
				f,
				"event migration version mismatch: expected 0, received {stored} (the log was written by a newer program; upgrade this client)"
			),
		}
	}
}

impl std::error::Error for ColourPixelsFlippedEventVersionError {}

impl ColourPixelsFlippedEvent {
	/// Decode one event record and tell stale logs apart from future ones. The failure message mirrors the generated JavaScript decoder.
	pub fn try_from_bytes(
		data: &[u8],
	) -> Result<&ColourPixelsFlippedEventZc, ColourPixelsFlippedEventVersionError> {
		let event = <Self as pina::PinaPodFixed>::read_exact(data)
			.map_err(|_| ColourPixelsFlippedEventVersionError::InvalidData)?;
		if event.discriminator != COLOUR_PIXELS_FLIPPED_EVENT_DISCRIMINATOR {
			return Err(ColourPixelsFlippedEventVersionError::InvalidData);
		}
		if event.migration_version > COLOUR_PIXELS_FLIPPED_EVENT_MIGRATION_VERSION {
			return Err(ColourPixelsFlippedEventVersionError::Future {
				stored: event.migration_version,
			});
		}
		Ok(event)
	}
}

/// Why `ColourPixelsFlippedEvent::project_from_bytes` could not produce current bytes.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ColourPixelsFlippedEventProjectionError {
	/// The bytes do not decode as this event's envelope.
	InvalidData,
	/// The record's payload length does not match the schema for its version.
	InvalidLength { stored: u32 },
	/// The log names this event but its adjacent transition is manual, so generated clients cannot project it.
	Manual { from: u32, to: u32 },
	/// The log names a version this client ships no checked-in projection for.
	Unknown { stored: u32 },
}

impl core::fmt::Display for ColourPixelsFlippedEventProjectionError {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		match self {
			Self::InvalidData => write!(f, "invalid ColourPixelsFlippedEvent event data"),
			Self::InvalidLength { stored } => write!(
				f,
				"event migration version mismatch: expected 0, received {stored} (the log length does not match the v{stored} schema)"
			),
			Self::Manual { from, to } => write!(
				f,
				"event migration version mismatch: expected 0, received {from} (the v{from} to v{to} transition is manual, so only an on-chain projection or a client generated from that schema can represent it)"
			),
			Self::Unknown { stored } => write!(
				f,
				"event migration version mismatch: expected 0, received {stored} (this client has no checked-in projection for it)"
			),
		}
	}
}

impl std::error::Error for ColourPixelsFlippedEventProjectionError {}

/// Current-shape event bytes paired with the version that actually wrote them.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ProjectedColourPixelsFlippedEvent {
	bytes: Vec<u8>,
	source_version: u8,
	was_migrated: bool,
}

impl ProjectedColourPixelsFlippedEvent {
	/// The exact current-version event bytes.
	#[must_use]
	pub fn bytes(&self) -> &[u8] {
		&self.bytes
	}

	/// The version carried by the immutable log record, matching the runtime's `CurrentEventData::source_version`.
	#[must_use]
	pub const fn source_version(&self) -> u8 {
		self.source_version
	}

	/// Whether a historical projection ran.
	#[must_use]
	pub const fn was_migrated(&self) -> bool {
		self.was_migrated
	}

	/// Decode the projected bytes as the current event shape.
	pub fn data(
		&self,
	) -> Result<&ColourPixelsFlippedEventZc, ColourPixelsFlippedEventVersionError> {
		ColourPixelsFlippedEvent::try_from_bytes(&self.bytes)
	}
}

/// Adjacent projections from the checked-in migration manifest: `(from, to, automatic, source payload size, destination payload size, moves)`.
#[allow(clippy::type_complexity)]
const COLOUR_PIXELS_FLIPPED_EVENT_PROJECTION_STEPS: &[(
	u32,
	u32,
	bool,
	usize,
	usize,
	&[(usize, usize, usize)],
)] = &[];

impl ColourPixelsFlippedEvent {
	/// Project current or historical event bytes into the current shape, mirroring the runtime's `normalize_event_data`.
	///
	/// Unknown, future, non-exact historical lengths, and manual transitions fail closed. The returned bytes always carry the current version and are decoded by [`Self::try_from_bytes`].
	pub fn project_from_bytes(
		data: &[u8],
	) -> Result<ProjectedColourPixelsFlippedEvent, ColourPixelsFlippedEventProjectionError> {
		if data.len() < 2 {
			return Err(ColourPixelsFlippedEventProjectionError::InvalidData);
		}
		if data[..1] != [1] {
			return Err(ColourPixelsFlippedEventProjectionError::InvalidData);
		}
		let mut version = u8::from_le_bytes(
			data[1..2]
				.try_into()
				.map_err(|_| ColourPixelsFlippedEventProjectionError::InvalidData)?,
		);
		let expected = COLOUR_PIXELS_FLIPPED_EVENT_MIGRATION_VERSION;
		if version > expected {
			return Err(ColourPixelsFlippedEventProjectionError::Unknown {
				stored: u32::from(version),
			});
		}
		let source_version = version;
		let mut payload = data[2..].to_vec();
		while version != expected {
			let Some((from, to, automatic, source_size, destination_size, moves)) =
				COLOUR_PIXELS_FLIPPED_EVENT_PROJECTION_STEPS
					.iter()
					.find(|(from, ..)| *from == u32::from(version))
			else {
				return Err(ColourPixelsFlippedEventProjectionError::Unknown {
					stored: u32::from(version),
				});
			};
			if !*automatic {
				return Err(ColourPixelsFlippedEventProjectionError::Manual {
					from: *from,
					to: *to,
				});
			}
			if payload.len() != *source_size {
				return Err(ColourPixelsFlippedEventProjectionError::InvalidLength {
					stored: u32::from(version),
				});
			}
			let mut destination = vec![0_u8; *destination_size];
			for (source_offset, destination_offset, size) in *moves {
				destination[*destination_offset..*destination_offset + *size]
					.copy_from_slice(&payload[*source_offset..*source_offset + *size]);
			}
			payload = destination;
			version = u8::try_from(*to)
				.map_err(|_| ColourPixelsFlippedEventProjectionError::Unknown { stored: *to })?;
		}
		let mut bytes = Vec::with_capacity(2 + payload.len());
		bytes.extend_from_slice(&COLOUR_PIXELS_FLIPPED_EVENT_DISCRIMINATOR.to_le_bytes()[..1]);
		bytes.extend_from_slice(&0u8.to_le_bytes()[..1]);
		bytes.extend_from_slice(&payload);
		Ok(ProjectedColourPixelsFlippedEvent {
			bytes,
			source_version,
			was_migrated: source_version != expected,
		})
	}
}

#[cfg(test)]
mod colour_pixels_flipped_event_projection_tests {
	use super::*;

	fn record(version: u8, payload: &[u8]) -> Vec<u8> {
		let mut data = vec![0_u8; 2 + payload.len()];
		data[..1].copy_from_slice(&[1]);
		data[1..2].copy_from_slice(&version.to_le_bytes()[..1]);
		data[2..].copy_from_slice(payload);
		data
	}

	#[test]
	fn future_versions_fail_closed() {
		let future: u8 = 1;
		let Err(error) = ColourPixelsFlippedEvent::project_from_bytes(&record(future, &[])) else {
			panic!("a 		 future version must fail");
		};
		assert_eq!(
			error,
			ColourPixelsFlippedEventProjectionError::Unknown {
				stored: u32::from(future)
			}
		);
	}

	#[test]
	fn wrong_lengths_and_discriminators_fail_closed() {
		let mut foreign = record(0, &[]);
		foreign[0] = foreign[0].wrapping_add(1);
		assert_eq!(
			ColourPixelsFlippedEvent::project_from_bytes(&foreign).err(),
			Some(ColourPixelsFlippedEventProjectionError::InvalidData),
		);
	}
}
