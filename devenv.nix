...
let
  ...
  # while the program now requires the 0.17 CLI (manifest format 4,
  # `[migrations].auto`). This mirrors the fork's `packages/pina/package.nix`
  # recipe against the official v0.17.0 release tarball. Drop this override
  # and return to `extra.pina` once the fork packages 0.17.0.
  pinaCli =
    let
      version = "0.17.0";
      platformSuffix =
...
in
{
  ...
  "generate:clients" = {
    exec = ''
      ...
      ${resolveFlutterSdk}
      CI=true pnpm install --frozen-lockfile
      pina generate --project "$DEVENV_ROOT/bitflip_program" --npx node
    ...
    '';
    description = "Generate the configured Pina clients (Dart, Rust, TypeScript, CPI, and CLI).";
    binary = "bash";
  };
    "generate:server" = {
      exec = ''
